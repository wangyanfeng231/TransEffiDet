...
from transformer.efficient_trans import EfficientDetBackbone
...
def fit_one_epoch(net, focal_loss, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda, old_loss):
    total_r_loss = 0
    total_c_loss = 0
    total_loss = 0
    val_loss = 0
    ...
    if (epoch + 1) % 10 == 0 and (epoch + 1) >= 70:
        torch.save(model.state_dict(), 'logs_cat4halfInput/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth' % (
        (epoch + 1), total_loss / (epoch_size + 1), val_loss / (epoch_size_val + 1)))
    if val_loss / (epoch_size_val + 1) < old_loss:
        torch.save(model.state_dict(), 'logs_cat4halfInput/loss.pth')
        old_loss = val_loss / (epoch_size_val + 1)
    return val_loss / (epoch_size_val + 1), old_loss
...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------#
    classes_path = 'model_data/ourclasses.txt'
    # ----------------------------------------------------#
    #   获取classes
    # ----------------------------------------------------#
    ...
    #   权值文件请看README，百度网盘下载
    # ------------------------------------------------------#
    # model_path = "logs/Epoch50-Total_Loss1.0731-Val_Loss0.8987.pth"
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, Freeze_Epoch):
            val_loss, old_loss = fit_one_epoch(net, efficient_loss, epoch, epoch_size, epoch_size_val, gen, gen_val, Freeze_Epoch,
                                     Cuda, old_loss)
    ...
    if True:
        ...
        for epoch in range(Freeze_Epoch, Unfreeze_Epoch):
            val_loss, old_loss = fit_one_epoch(net, efficient_loss, epoch, epoch_size, epoch_size_val, gen, gen_val,
                                     Unfreeze_Epoch, Cuda, old_loss)
            lr_scheduler.step(val_loss)
